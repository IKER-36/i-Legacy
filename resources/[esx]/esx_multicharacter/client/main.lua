...
if ESX.GetConfig().Multichar then
	...
	function CharacterOptions(Characters, slots, SelectedCharacter)
		local elements = {{title = "Personaje: ".. Characters[SelectedCharacter.value].firstname .. " ".. Characters[SelectedCharacter.value].lastname,icon = "fa-regular fa-user", unselectable = true}, 
		{title = "Return", unselectable = false,icon = "fa-solid fa-arrow-left",description ="Volver al menú de personajes.", action = "return"}}
		if not Characters[SelectedCharacter.value].disabled then 
			elements[3] = {title = _('char_play'), description ="Entrar a la Ciudad.", icon ="fa-solid fa-play",action = 'play', value = SelectedCharacter.value}
		else
		...
		if Config.CanDelete then elements[4] = {title = _('char_delete'),icon ="fa-solid fa-xmark",description ="Eliminar permanentemente este personaje.", action = 'delete', value = SelectedCharacter.value} end
